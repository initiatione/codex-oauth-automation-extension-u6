...
- SUB2API 来源配置中，`分组` 下方提供 `优先级` 数字输入；默认 `1`，只允许保存大于等于 `1` 的整数，并沿用 SUB2API 的语义：数值越小，账号优先级越高。自动流程运行时该输入会随其它 SUB2API 来源配置一起锁定，避免当前轮中途改写授权后创建账号的 payload。
...
- 白嫖复用手机号记录 `freeReusablePhoneActivation`：用于保存已收到短信验证码后的 HeroSMS 号码，跨轮次保留，直到用户清除、达到复用上限、目标站拒绝该号码，或自动复用流程判定应退休
- localhost 回调地址
- 自动运行轮次信息
...
- CPA / SUB2API 配置；SUB2API 额外持久化 `sub2apiAccountPriority`，默认 `1`，用于授权完成后创建账号时写入上游兼容字段 `priority`
- Codex2API 配置
...
- 白嫖复用开关 `freePhoneReuseEnabled / freePhoneReuseAutoEnabled`：默认开启；前者保存并允许复用已接收短信验证码的 HeroSMS 号码，后者允许后台自动把保存的号码重新置为等待短信状态并继续自动提交验证码
- iCloud 相关偏好
- LuckMail API 配置
- 自动运行默认配置
- Step 6 登录前 Cookie 清理等待秒数 `step6CookieClearDelaySeconds`：默认 `25`，允许 `0~300`，运行时需要暂停后修改；后台每次执行 Step 6 前读取最新持久配置
...
1. 从持久配置读取 `step6CookieClearDelaySeconds` 并等待；默认 25 秒，允许 0~300 秒
...
- sidepanel 在主配置区提供“第六步清 Cookie”数字输入；修改会实时保存，但自动流程运行中会禁用，需要暂停后再改。
- 后台不会沿用启动时的旧值；每次执行 `runPreStep6CookieCleanup()` 前都会重新读取最新持久配置，因此下一次执行 Step 6 时生效。
...
- HeroSMS 白嫖复用开启后，后台在首次成功收到短信验证码时会保存当前号码到 `freeReusablePhoneActivation`，并跳过该激活的完成/取消/封禁等终态 `setStatus(6/8)`，避免把已拿到验证码的号码释放掉。
- 自动白嫖复用开启时，Step 9 会优先使用 `freeReusablePhoneActivation`，调用 HeroSMS `setStatus(3)` 后轮询 `getStatus/getStatusV2`，确认进入 `STATUS_WAIT_CODE / STATUS_WAIT_RETRY / STATUS_WAIT_RESEND` 等等待短信状态后才向 OpenAI 页面提交该号码；若准备失败、被取消或始终无法进入等待状态，本次会停止且不购买新 HeroSMS 号码。
- 自动白嫖复用成功后会累计 `successfulUses`，达到 `maxUses` 后清除本地记录；若目标站明确提示号码不可用、已达上限或无法发送短信，会从白嫖复用记录和普通复用池中移除该号码，避免后续继续复用坏号码。
- 如果只手动记录了手机号但没有 HeroSMS activation ID，自动复用无法重新拉起接码订单；流程只会填入该号码并停止，提示用户到 SMS 侧手动刷新。
...
- sidepanel 保存的 `sub2apiAccountPriority` 会在步骤 7 申请 OAuth URL 和步骤 10 平台验证时一路透传到 `content/sub2api-panel.js`。
- OAuth 授权完成后，SUB2API 内容脚本创建账号时把该值写入请求体的 `priority` 字段；缺失值按 `1` 处理，显式传入 `0`、小数或非数字会被拒绝，避免向 SUB2API 提交无效优先级。
...
7. iCloud 邮箱验证码轮询会优先读取可见首行邮件中的验证码；如果该验证码已经在当前 Step 4 / Step 8 会话里提交过，则继续等待新的匹配邮件，不会把已排除验证码反复提交。
...
- `withIcloudLoginHelp` 只能在函数入口初始化一次 `safeActionLabel`；瞬态上下文错误分支复用这个变量，避免进入 `Cannot access 'safeActionLabel' before initialization` 这类 TDZ 崩溃。
...
- PR 提交、diff 清理、分支基线或最终反馈要求变更
  更新 [开发者AI开发与PR提交流程.md](c:/Users/projectf/Downloads/codex注册扩展/开发者AI开发与PR提交流程.md)
